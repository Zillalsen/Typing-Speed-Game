...
//set charindex and mistaks to 0
...
function initTyping() {
  ...
  if (charIndex < characters.length - 1 && timeLeft > 0) {
    // once timer is start , it Won't restart again on every click
    if (!isTyping) {
      timer = setInterval(Counter, 1000);
      isTyping = true;
    }
    // if the user use backspace or hasn't enterd any Chars
    if (typedChar == null) {
      charIndex--;
      // decrement mistake only if the characters has the class incorrect
      if (characters[charIndex].classList.contains("incorrect")) {
        mistakes--;
      }
      characters[charIndex].classList.remove("correct", "incorrect");
    } else {
      //if the letters in paragraph matches letters in input feild then add class correct
      //else add the incorrect class and icremment mistaks
    ...
    // if wpm equals to infinity or less then 0 set its value to 0;
    wpm = wpm < 0 || wpm === Infinity || !wpm ? 0 : wpm;
    wpmTag.innerText = wpm;
    cpmTag.innerText = charIndex - mistakes; // cpm doesn't count mistake
  } else {
    inpField.value = "";
    clearInterval(timer);
  }
